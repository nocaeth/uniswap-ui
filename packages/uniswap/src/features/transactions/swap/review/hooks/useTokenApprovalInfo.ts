...
import { getGnosisVeloraApprovalSpender } from 'uniswap/src/features/transactions/swap/services/gnosisRouter/velora'
...
export function useTokenApprovalInfo(params: TokenApprovalInfoParams): ApprovalTxInfo {
  const { address, chainId, wrapType, currencyInAmount, currencyOutAmount, routing, tradeType, quote } = params
  ...
  const gnosisApprovalInfo = useGnosisApprovalInfo({
    ...
    currencyOutAmount,
    tradeType,
    approvalSpenderOverride: getGnosisVeloraApprovalSpender(quote),
  })
