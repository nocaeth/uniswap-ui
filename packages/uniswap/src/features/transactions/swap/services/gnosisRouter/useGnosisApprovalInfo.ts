...
// Static gas allowance (gas units) for the ERC20 -> Permit2 approve tx, used for the
// network-cost estimate (the wallet re-estimates at submission).
const ERC20_APPROVE_GAS = 55_000

/**
 * Client-side ERC20 → Permit2 approval status for Gnosis, replacing the Trading API's
 * `checkApproval` (which doesn't serve Gnosis). The Permit2 → UniversalRouter step is
 * handled separately via the quote's `permitTransaction` (see fetchGnosisQuote).
 */
export function useGnosisApprovalInfo(params: {
  ...
  const { address, chainId, wrapType, currencyInAmount } = params
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: ['gnosisErc20Allowance', chainId, address, tokenAddress],
    enabled,
    ...
    queryFn: async (): Promise<{ allowance: string; gasPrice: string }> => {
      const [allowance, gasPrice] = await Promise.all([
        readErc20Allowance({ owner: address as string, token: tokenAddress as string, spender: PERMIT2_ADDRESS }),
    ...
    return {
      tokenApprovalInfo: {
        ...
        txRequest: {
          ...
          from: address,
          chainId,
          data: buildErc20ApproveData(PERMIT2_ADDRESS),
        },
  ...
  }, [address, chainId, data, error, gasStrategy, isLoading, isNative, isWrap, requiredAmount, tokenAddress])
}
