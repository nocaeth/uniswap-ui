...
import { GNOSIS_TOKEN_LIST, type GnosisTokenListToken } from 'uniswap/src/features/tokens/gnosisTokenList'
...
function buildGnosisTokenListCurrencyInfo(tokenInfo: GnosisTokenListToken): CurrencyInfo {
  return buildPartialCurrencyInfoWithLogo(
    new Token(UniverseChainId.Gnosis, tokenInfo.address, tokenInfo.decimals, tokenInfo.symbol, tokenInfo.name),
    tokenInfo.logoURI,
  )
}
