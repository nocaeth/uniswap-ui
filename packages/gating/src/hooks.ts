...
import { getPinnedFeatureFlagValue } from '@universe/gating/src/pinnedFeatureFlags'
import {
  getStatsigClient,
  TypedReturn,
  useDynamicConfig,
...
export function useFeatureFlag(flag: FeatureFlags): boolean {
  ...
  return getPinnedFeatureFlagValue(flag) ?? value
}
...
export function useFeatureFlagWithLoading(flag: FeatureFlags): { value: boolean; isLoading: boolean } {
  ...
  return { value: getPinnedFeatureFlagValue(flag) ?? value, isLoading: isStatsigLoading }
}

export function getFeatureFlag(flag: FeatureFlags): boolean {
  const pinnedValue = getPinnedFeatureFlagValue(flag)
